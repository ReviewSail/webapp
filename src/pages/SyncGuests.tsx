  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-ink">Import Guests</h2>
      ...
      {/* Location warning */}
      {!activeLocationId && (
        <div className="flex items-center gap-3 p-4 rounded-lg bg-caution-soft border border-caution/20 text-caution text-sm">
        ...
        <div className="bg-card rounded-xl border border-line p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="John"
                  className="w-full px-3 py-2 border border-line rounded-lg text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-colors"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  disabled={!activeLocationId}
                />
                <p className="text-xs text-ink-faint mt-1">Optional — enables mid-stay check-ins</p>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loading ? (
                <>
      ...
      {feedback && (
        <div className={`p-4 rounded-lg text-sm flex items-start gap-3 ${
          feedback.type === 'success' ? 'bg-positive-soft text-positive border border-positive/20' : 'bg-critical-soft text-critical border border-critical/20'
        }`}>
      ...
      {isGuideOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setIsGuideOpen(false)}>
          <div
            className="bg-card rounded-2xl shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto"
            onClick={e => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-card border-b border-line px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <h3 className="text-lg font-semibold text-ink flex items-center gap-2">
                <BookOpen size={20} className="text-brand-500" />
                CSV Import Format Guide
              </h3>
              <button onClick={() => setIsGuideOpen(false)} className="p-1 hover:bg-muted rounded-lg transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="border border-line rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 bg-canvas hover:bg-muted transition-colors"
                >
                  <span className="font-medium text-sm text-ink">Booking.com Export</span>
                  {openSection === 'booking' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
                {openSection === 'booking' && (
                  <div className="px-4 py-3 space-y-2 text-sm text-ink-muted">
                    <p>Export your reservations from Booking.com as CSV and upload directly.</p>
                    <p className="text-xs text-ink-faint">Supported columns: Guest first name, Guest last name, Email, Phone, Check-out, Check-in</p>
                  </div>
                )}
              </div>
              ...
              <div className="border border-line rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 bg-canvas hover:bg-muted transition-colors"
                >
                  <span className="font-medium text-sm text-ink">Airbnb Export</span>
                ...
                {openSection === 'airbnb' && (
                  <div className="px-4 py-3 space-y-2 text-sm text-ink-muted">
                    ...
                    <p className="text-xs text-ink-faint">Supported columns: First Name, Last Name, Email, Phone Number, Checkout, Check-in</p>
                  </div>
                )}
              </div>
              ...
              <div className="border border-line rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 bg-canvas hover:bg-muted transition-colors"
                >
                  <span className="font-medium text-sm text-ink">Expedia / VRBO Export</span>
                ...
                {openSection === 'expedia' && (
                  <div className="px-4 py-3 space-y-2 text-sm text-ink-muted">
                    ...
                    <p className="text-xs text-ink-faint">Supported columns: First Name, Last Name, Email, Phone, Departure Date, Arrival Date</p>
                  </div>
                )}
              </div>

              {/* Custom format */}
              <div className="border border-line rounded-lg p-4 bg-canvas">
                <h4 className="font-medium text-sm text-ink mb-2">Custom CSV Format</h4>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <span className="text-ink-muted ml-1">firstname, first name, guest name</span>
                  </div>
                  <div>
                    <span className="font-medium text-ink">Last Name:</span>
                    <span className="text-ink-muted ml-1">lastname, last name</span>
                  </div>
                  <div>
                    <span className="font-medium text-ink">Email:</span>
                    <span className="text-ink-muted ml-1">email, email address, e-mail</span>
                  </div>
                  <div>
                    <span className="font-medium text-ink">Phone:</span>
                    <span className="text-ink-muted ml-1">phone, phone number, mobile, tel</span>
                  </div>
                  <div>
                    <span className="font-medium text-ink">Check-out:</span>
                    <span className="text-ink-muted ml-1">checkout, departure, end date</span>
                  </div>
                  <div>
                    <span className="font-medium text-ink">Check-in:</span>
                    <span className="text-ink-muted ml-1">checkin, arrival, start date</span>
                  </div>
                </div>
              </div>
            </div>
